{
  ...
  DE = "${custom.x0.derived.deSelected}";
in
      ...
      case $DE_CONS in
         hyprland)
           if pgrep -x hyprpaper >/dev/null 2>&1; then
             ...
             echo "[wallust-pick] hyprpaper not running; start it or manage via your Hyprland config." >&2
           fi
           ;;
         awesomewm)
           echo "[wallust-pick] applying via nitrogen…"
           # --save writes to ~/.config/nitrogen/bg-saved.cfg so it persists across restarts
           nitrogen --set-zoom-fill "''${SEL}" --save || true
           ;;
         *)
           echo "[wallust-pick] unknown DE '${DE}', falling back to feh…"
           feh --bg-fill "''${SEL}" || true
           ;;
       esac
